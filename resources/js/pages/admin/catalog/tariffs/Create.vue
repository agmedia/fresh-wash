...
const form = useForm({
    service_id: (props.services?.[0]?.id ?? null) as number | null,
    location_id: (props.locations?.[0]?.id ?? null) as number | null,
    price_net: '',
    vat_rate: String(props.defaults?.vat_rate ?? 25),
    price_gross: '',
...
const selectedLocation = computed(() =>
    props.locations.find((l) => l.id === form.location_id),
);
...
const calcGross = () => {
    ...
    return net ? (Math.round(gross * 100) / 100).toFixed(2) : '';
};
...
const submit = () => {
    form.post('/admin/catalog/tariffs', { preserveScroll: true });
};
</script>
                ...
                <div class="flex flex-wrap items-center justify-between gap-3 rounded border border-border bg-card p-3 card-elev">
                    <div class="min-w-0">
                        ...
                        <div class="text-xs text-muted-foreground">
                            {{ selectedService?.name ?? 'Usluga' }} · {{ selectedLocation?.name ?? 'Lokacija' }}
                        </div>
                    </div>
            ...
            >
                        ...
                        <div class="space-y-1.5">
                            ...
                            <select
                                ...
                                class="w-full rounded border border-input bg-background px-3 py-2 text-sm text-foreground
                                       focus:outline-none focus:ring-2 focus:ring-ring/40"
                            >
                                <option :value="null" disabled>Odaberi uslugu…</option>
                ...
                <div class="flex items-center justify-end gap-3 pt-2">
                    <button
                        ...
                        :disabled="processing"
                    >
                        {{ processing ? 'Spremanje…' : 'Kreiraj tarifu' }}
                    </button>
                </div>
            </form>
        </div>
